...
const wordPairs = {
  ...
  "Aventurier": "Sac à dos",
  "Camp": "Tente",
  "Feu de camp": "Guimauve",
  ...
  "Abeille": "Guêpe",
  "Papillon": "Mite",
  "Tortue de mer": "Tortue terrestre",
  "Serpent": "Anguille",
  "Chimpanzé": "Gorille",
  "Éléphant": "Mammouth",
  "Crocodile": "Alligator",
  ...
  "Coiffeur": "Barbier",
  "Pilote d'avion": "Pilote d'hélicoptère",
  "Pêcheur": "Marin",
  "Facteur": "Livreur",
  ...
  "Illusionniste": "Hypnotiseur",
  "Boucher": "Charcutier",
  "Psychologue": "Psychiatre",
  "Opticien": "Audioprothésiste",

  // === Vêtements ===
  "Jean": "Pantalon en toile",
  "Chemise": "Chemisier",
  "Manteau": "Doudoune",
  "Jupe": "Jupe-culotte",
  "Botte": "Bottine",
  ...
  // === Technologie ===
  "Smartphone": "Tablette",
  "Enceinte Bluetooth": "Barre de son",
  "Appareil photo": "Caméra",
  "Télévision": "Vidéoprojecteur",
  "PlayStation": "Xbox",
  "Drone": "Hélicoptère télécommandé",
  ...
  "Tramway": "Métro",
  "Scooter": "Mobylette",
  "Trottinette électrique": "Hoverboard",
  ...
  // === 50 NOUVELLES PAIRES SIMILAIRES ===

  // Très similaires - super tricky
  "Confetti": "Paillettes",
  "Trampoline": "Château gonflable",
  "Cerise": "Griotte",
  ...
  // Objets & concepts proches
  "Valse": "Tango",
  "Salsa": "Bachata",
  "Hip-hop": "Breakdance",
  "Graffiti": "Tag",
  "Tatouage": "Henné",
  "Piercing": "Boucle d'oreille",
  "Sablier": "Chronomètre",
  "Boussole": "GPS",
  "Jumelles": "Télescope",
  "Loupe": "Microscope",
  "Dictionnaire": "Encyclopédie",
  "Atlas": "Globe terrestre",
  "Puzzle": "Casse-tête",
  "Échecs": "Dames",
  "Domino": "Mahjong",
  "Loto": "Bingo",
  "Fléchettes": "Tir à l'arc",
  "Pétanque": "Bowling",

  // Lieux & ambiances similaires
  "Carnaval": "Festival",
  "Concert": "Spectacle",
  "Cirque": "Cabaret",
  "Aquaparc": "Piscine à vagues",
...
function getActivePlayerCount(roomCode) {
  const room = rooms[roomCode];
  if (!room) return 0;
  return room.players.filter(player => player.turnOrder !== null && player.turnOrder !== undefined).length;
}

function broadcastSpeakerUpdate(roomCode, targetSocketId) {
  const room = rooms[roomCode];
  if (!room) return;
  const playerList = getPlayerList(roomCode);
  const speaker = playerList.find(p => p.turnOrder === room.currentSpeaker);
  const payload = {
  ...
  if (targetSocketId) {
    io.to(targetSocketId).emit('speakerUpdate', payload);
  } else {
    io.to(roomCode).emit('speakerUpdate', payload);
  }
}
...
function removePlayerFromRoom(roomCode, playerId, options = {}) {
  ...
  if (room.gameStarted && room.currentSpeaker === removedPlayer.turnOrder) {
    ...
    if (nextSpeaker) {
      ...
      broadcastSpeakerUpdate(roomCode);
    }
  }
  ...
  socket.on('joinRoom', (data) => {
    ...
    if (existingPlayerIndex !== -1) {
      ...
      if (room.gameStarted) {
        ...
        if (room.playerWords?.[player.name]) {
        ...
        broadcastSpeakerUpdate(roomCode, socket.id);
      }
    ...
    for (const roomCode in rooms) {
      ...
      if (playerIndex !== -1) {
        ...
        if (room.gameStarted || wasHost) {
          ...
          // Game hasn't started, keep them in the player list so the lobby stays intact
          if (rooms[roomCode]) {
            io.to(roomCode).emit('playerListUpdate', { players: getPlayerList(roomCode) });
          }
        }
